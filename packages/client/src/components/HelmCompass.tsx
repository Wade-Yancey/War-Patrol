...
interface Props {
  ...
  turnRate?: number;
  /** Side-panel actions (TouchNumber + submit) — same aux column as hydrophone. */
  children?: ReactNode;
}
...
function HelmCompassInner({
  ...
  return (
    <div
      className={`radar-scope radar-console crt-console helm-compass${
        interactive ? ' helm-compass--interactive' : ''
      }${disabled ? ' helm-compass--disabled' : ''}`}
    >
      <div className="radar-scope-plot">
        <svg
          ref={svgRef}
          className="radar-scope-svg helm-compass-svg"
      ...
      <div className="radar-side-panel helm-compass-aux">
        <div className="crt-console-readouts helm-compass-readouts">
          <div className="crt-console-readout helm-compass-readout">
            <span className="crt-console-key helm-compass-key">HDG</span>
            <span className="readout crt-console-val helm-compass-val">{hdgLabel}°</span>
          </div>
          <div className="crt-console-readout helm-compass-readout">
            <span className="crt-console-key helm-compass-key">CRS</span>
            <span className="readout crt-console-val helm-compass-val">{crsLabel}°</span>
            {onCourse && <span className="helm-compass-ok">ON</span>}
          </div>
          {draftLabel && (
            <div className="crt-console-readout helm-compass-readout helm-compass-readout--draft">
              <span className="crt-console-key helm-compass-key">SET</span>
              <span className="readout crt-console-val helm-compass-val">{draftLabel}°</span>
            </div>
          )}
        </div>
        ...
        {children && <div className="helm-compass-actions">{children}</div>}
